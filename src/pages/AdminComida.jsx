...
export default function AdminComida() {
  ...
  const urlAddComida = "http://10.0.0.47:8000/api/addComida";
  const urlAddOpcionDia = "http://10.0.0.47:8000/api/addOpcion";
  const urlBuscarComida = "http://10.0.0.47:8000/api/busquedaPorDia";
  const urlExportarComida = "http://10.0.0.47:8000/api/exportexcel";
  const urlCantComidas = "http://10.0.0.47:8000/api/contadores";
  const urlOpcionesComidas = "http://10.0.0.47:8000/api/busquedaComidas";
  const urlDeshabilitarMes = "http://10.0.0.47:8000/api/updateSeleFalse";
  const urlHabilitarMes = "http://10.0.0.47:8000/api/updateSeleTrue";
  const urlBorrarComidaSelecUser = "http://10.0.0.47:8000/api/deleteMenu";
  const urlBorrarComidaMes = "http://10.0.0.47:8000/api/deleteComidaMes";
  const urlBorrarComidaDiaMes = "http://10.0.0.47:8000/api/deleteComidaSele";
  ...
  const [ubiArchivo, setUbiArchivo] = useState("")

  const asd2 = useColorModeValue(
    "2xl",
    "0px 0px 13px 2px rgba(255,255,255,0.4)"
  );
  ...
  const exportarComida = async () => {
    await axios
      .post(urlExportarComida, {
        dia: formatDia,
        mes: formatMes,
        año: formatAño,
      })
      .then((res) => {
        setUbiArchivo(`.././comida ${formatDia}-${formatMes}-${formatAño}.xlsx`)
        if (res.data.archivo === false) {
          toast({
  ...
  return (
    <>
      ...
      <Box minH="100vh">
        ...
        <Heading fontSize={[25, 35, 45, 60]} my="3%" ml="7%">
          Cargar Comidas
        </Heading>
        <Flex ml="8%" wrap="wrap">
          <Button onClick={handlePag} boxShadow={asd2} mx="10px" mb="2%">
            Cargar Comidas del distribuidor
          </Button>
          <Button onClick={handlePag2} boxShadow={asd2} mx="10px" mb="2%">
            Cargar menus por dia
          </Button>
          <Button onClick={handlePag3} boxShadow={asd2} mx="10px" mb="2%">
            Habilitar-deshabilitar
          </Button>
          <Button onClick={handlePag4} boxShadow={asd2} mx="10px" mb="2%">
            Eliminar menus por dia
          </Button>
          <Button onClick={handlePag5} boxShadow={asd2} mx="10px" mb="2%">
            Eliminar mes guardado por usuarios
          </Button>
        </Flex>
                  ...
                  <Stack divider={<StackDivider />} spacing="4">
                    <Box>
                      <Textarea
                        w="100%"
                        id="comidaAdd"
                        onChange={(e) => setComidaAdd(e.target.value)}
                      ></Textarea>
                    </Box>
                    <Box align="center">
                      <Button
          ...
          <Box minW="50%">
            <Card mt="15px" boxShadow={asd2}>
                      ...
                      <Tr>
                        ...
                        <Td>
                          <Button
                            colorScheme="red"
                            size="sm"
                            onClick={() => seleccionarComida(comDia)}
                          >
              ...
              <Button
                colorScheme="green"
                size="lg"
                onClick={() => exportarComida()}
              >
                Generar Excel
              </Button>
              {ubiArchivo != "" ? <Flex justify="center"><DescargaExcel ubicacion={ubiArchivo}/></Flex> : <></>}
            </Card>
          </Box>
        </Flex>
      </Box>
