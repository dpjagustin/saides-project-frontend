...
export default function Noticias() {
  ...
  const urlComidaDia = "http://10.0.0.47:8000/api/busquedaPorPersonayDia";
  const urlauth = "http://10.0.0.47:8000/api/authUser";
  const [data, setData] = useState([]);
  const cambiarColoresShadow = useColorModeValue(
    "xl",
    "0px 0px 8px 2px rgba(255,255,255,0.5)"
  );
  const [diaLimpio, setDiaLimpio] = useState("");
  const [mesLimpio, setMesLimpio] = useState("");
  const [añoLimpio, setAñoLimpio] = useState("");
  const hoy = new Date();
  const [data3, setData3] = useState();
  const [nombrecompleto, setNombrecompleto] = useState("");
  const [sinUsuario, setSinUsuario] = useState(true);

  ///////////// TRAE LAS NOTICIAS///////////////
  useEffect(() => {
    setDiaLimpio(hoy.getDate());
    setMesLimpio(hoy.getMonth() + 1);
    setAñoLimpio(hoy.getFullYear());
    (async () => {
      await axios
        .get(urlnoticias)
        .then((res) => {
  ...
  return (
    <>
      <LineaDivisora />
      <Heading fontSize={[25, 35, 45, 60]} ml="7%">
        Noticias
      </Heading>
      <Flex
        w="100%"
        ...
        <Flex wrap="wrap" w="90%" justify="center">
          ...
          {data.map((nov) => (
            <Box
              rounded="md"
              key={nov.id}
              boxShadow={cambiarColoresShadow}
              minW="300px"
              minHeight="200px"
              maxW="400px"
              overflow="hidden"
